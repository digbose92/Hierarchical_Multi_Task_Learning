...
%% Code starts here
function [W, C, funcVal,fval_list] = Logistic_Lasso(X, Y, rho1, opts)
...
while iter < opts.maxIter
    ...
    t_old = t;
    t = 0.5 * (1 + (1+ 4 * t^2)^0.5);
    fval_list=[fval_list,funcVal(end)];
    iteration_list=[iteration_list,funcVal(end)];
    fprintf('\n Iteration:%d,Function Value:%d',iter,funcVal(end));
end
plot(fval_list);
W = Wzp;
C = Czp;
